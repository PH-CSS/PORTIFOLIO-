...
for (let i = 0; i < 5; i++) {
  arrRespostas.push(Cookies.get(`Resp${Cookies.get("mode")+(i+1)}`));
  arrGab.push(Cookies.get(`gabarito${i+1}`));
...
for (let i = 1; i <= 5; i++) {
  ...
  if (Cookies.get(`Resp${Cookies.get("mode")}${i}`) === Cookies.get(`gabarito${i}`)) {
    ...
    correta.innerHTML = `Correta: <br> ${Cookies.get(`gabarito${i}`)}`;
