...
function atualizar() {
  ...
  tr.children[2].children[0].addEventListener("focusout", () => {
    ...
    if (pacienteObj.altura > 100) {
          ...
          if (!Number(conta) || conta == Infinity) {
            confirm("Alguma informação está incorreta!");
            if (confirm("Alguma informação está incorreta!") == true) tr.children[2].children[0].focus();
          }else(
